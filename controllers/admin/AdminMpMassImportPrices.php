...
class AdminMpMassImportPricesController extends ModuleAdminController
{
    ...
    private function initHelperList()
    {
        ...
        $this->_select = "a.id_product as thumb, '--' as manufacturer, p.wholesale_price as old_wholesale_price, p.price as old_price";
    ...
    public function addVatCompareWholesalePrice($value, $row)
    {
        $tax_rate = 22;

        $price_old_ti = $row['old_wholesale_price'] * (100 + $tax_rate) / 100;
        $price_ti = $value * (100 + $tax_rate) / 100;

        $displayOldPrice = Tools::displayPrice($price_old_ti);
        $displayPrice = Tools::displayPrice($price_ti);

        if ($displayOldPrice != $displayPrice) {
            return "<span class=\"badge badge-warning\">$displayPrice</span>";
        }

        return $displayPrice;
    }

    public function addVatComparePrice($value, $row)
    {
        $tax_rate = 22;

        $price_old_ti = $row['old_price'] * (100 + $tax_rate) / 100;
        $price_ti = $value * (100 + $tax_rate) / 100;
